...
shaka.net.NetworkingEngine = class extends shaka.util.FakeEventTarget {
   ...
   * @param {shaka.net.NetworkingEngine.OnRequestEnded=} onRequestEnded
   *   Called when a request is ended
  ...
  constructor(onProgressUpdated, onHeadersReceived, onDownloadFailed,
      onRequest, onRequestEnded, onRetry) {
    super();
    ...
    /** @private {?shaka.net.NetworkingEngine.OnRequestEnded} */
    this.onRequestEnded_ = onRequestEnded || null;
  ...
  send_(type, request, context, backoff, index, lastError,
    ...
    const sendOperation = backoffOperation.chain(() => {
      ...
      if (this.onRequestEnded_) {
        this.onRequestEnded_(type, request, context);
      }
 ...
 *
 * @description
 * A callback function called on every request's end
 * @export
 */
shaka.net.NetworkingEngine.OnRequestEnded;
