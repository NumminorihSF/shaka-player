...
goog.require('shaka.abr.EwmaTtfbEstimator');
...
shaka.abr.SimpleAbrManager = class {
  ...
  constructor() {
    ...
    this.switch_ = null;
    /** @private {?shaka.extern.AbrManager.GetSegmentDuration} */
    this.getSegmentDuration_ = null;
    ...
    /** @private {shaka.abr.EwmaTtfbEstimator} */
    this.ttfbEstimator_ = new shaka.abr.EwmaTtfbEstimator();
      ...
      this.onNetworkInformationChange_ = () => {
        if (this.enabled_ && this.config_.useNetworkInformation) {
          ...
          if (this.config_) {
            ...
            this.ttfbEstimator_.configure(this.config_.advanced);
          }
  ...
  init(switchCallback, getSegmentDuration) {
    ...
    this.getSegmentDuration_ = getSegmentDuration;
  }
  ...
  chooseVariant(preferFastSwitching = false) {
    ...
    const defaultTtfbEstimate = this.getDefaultTtfb_();
    const currentTtfb = this.ttfbEstimator_.getTtfbEstimate(
        defaultTtfbEstimate) / 1000;
    ...
    for (let i = 0; i < sortedVariants.length; i++) {
      ...
      const duration = this.getSegmentDuration_ ?
    this.getSegmentDuration_(item) : null;
      const durationMultiplier = duration ? duration /
          (duration - currentTtfb) : 1;
      const playbackRate =
          !isNaN(this.playbackRate_) ? Math.abs(this.playbackRate_) : 1;
      const itemBandwidth = playbackRate * item.bandwidth * durationMultiplier;
      const minBandwidth =
      ...
      const nextDuration = this.getSegmentDuration_(next);
      const nextDurationMultiplier = nextDuration ? duration /
          (duration - currentTtfb) : 1;
      const nextBandwidth = playbackRate * next.bandwidth *
          nextDurationMultiplier;
   ...
   */
  sampleTtfb(ttfbMs) {
    shaka.log.v2('Ttfb sampled:',
        'ttfbMs=' + ttfbMs,
        'lastTimeChosenMs=' + this.lastTimeChosenMs_,
        'enabled=' + this.enabled_);
    goog.asserts.assert(ttfbMs >= 0, 'expected a non-negative duration');
    this.ttfbEstimator_.sample(ttfbMs);
  }
  ...
  configure(config) {
    ...
    if (this.ttfbEstimator_ && this.config_) {
      this.ttfbEstimator_.configure(this.config_.advanced);
    }
  }
   ...
   */
  getDefaultTtfb_() {
    let defaultTtfbEstimate = this.config_.defaultTtfbEstimate;

    // Some browsers implement the Network Information API, which allows
    // retrieving information about a user's network connection.
    if (navigator.connection && navigator.connection.rtt &&
        this.config_.useNetworkInformation) {
      // If it's available, get the ttfb estimate from the browser (in
      // milliseconds) and use it as defaultTtfbEstimate.
      defaultTtfbEstimate = navigator.connection.rtt;
    }
    return defaultTtfbEstimate;
  }
