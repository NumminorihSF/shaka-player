...
goog.require('shaka.abr.ProgressAccumulator');
...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, dependencyInjector) {
    ...
    /** @private {!shaka.abr.ProgressAccumulator} */
    this.progressAccumulator_ = new shaka.abr.ProgressAccumulator(
  ...
  async destroy() {
    ...
    this.progressAccumulator_.destroy();
  ...
  createNetworkingEngine() {
    ...
    const onProgressUpdated_ = (deltaTimeMs, bytesDownloaded, allowSwitch) => {
      this.progressAccumulator_.handleProgress(bytesDownloaded, allowSwitch);
    };
    /** @type {shaka.net.NetworkingEngine.OnHeadersReceived} */
    const onHeadersReceived_ = (headers, request, requestType,
        timeToHeaders) => {
      this.progressAccumulator_.responseStart();
      if (this.abrManager_) {
        this.abrManager_.sampleTtfb(timeToHeaders);
      }
    ...
    /** @type {shaka.net.NetworkingEngine.OnRequestEnded} */
    const onRequestEnded_ = () => {
      this.progressAccumulator_.requestEnded();
    };
  ...
  applyConfig_() {
    ...
    if (this.progressAccumulator_) {
      this.progressAccumulator_.configure(
          this.config_.abr.advanced.progressInterval);
    }
  }
